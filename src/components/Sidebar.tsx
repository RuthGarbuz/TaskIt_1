...
export default function Sidebar({ currentView, onViewChange, onProjectSelect, onLogout, projects }: SidebarProps) {

  const [projectsExpanded, setProjectsExpanded] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');

  const menuItems = [
     { id: 'settings' as CurrentView, label: 'הגדרות', icon: Settings },
    { id: 'myTasks' as CurrentView, label: 'משימות שלי', icon: ListTodo },
    { id: 'allTasks' as CurrentView, label: 'כל המשימות', icon: CheckSquare },
    { id: 'hoursReport' as CurrentView, label: 'דיווחי שעות', icon: Clock,       activeColor: 'bg-teal-500'    },
    { id: 'workload' as CurrentView,    label: 'עומס עבודה',  icon: TrendingUp,  activeColor: 'bg-indigo-500'  },
  ];
