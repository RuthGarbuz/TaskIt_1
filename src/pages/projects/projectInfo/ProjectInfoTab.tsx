...
function ReadField({ label, value, icon: Icon, colSpan }: {
  ...
  return (
    <div className={`space-y-1 ${colSpan ? 'col-span-2' : ''}`}>
      ...
      <div className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-700 min-h-[38px] flex items-center">
...
<div className={`grid gap-5 ${permissionId && permissionId > 2 ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-2'}`}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 bg-gray-50 border-b border-gray-200 flex items-center gap-2">
            ...
            <span className="font-bold text-gray-600 text-sm">פרטי פרויקט</span>
            {/* פעיל / לא פעיל badge */}
            <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${
              projectInfo?.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'
            }`}>
              {projectInfo?.isActive ? 'פעיל' : 'לא פעיל'}
            </span>
            <span className="mr-auto text-[11px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded">קריאה בלבד</span>
          </div>
