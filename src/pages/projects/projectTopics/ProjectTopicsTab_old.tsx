...
function DateInput({
  ...
  return (
    <div className={`relative border border-gray-300 rounded-lg bg-white overflow-hidden focus-within:ring-2 ${ringColor} ${className}`}>
      <input
        type="date"
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full px-1.5 py-1.5 text-xs bg-transparent focus:outline-none cursor-text"
      />
    </div>
  );
}
...
// ─── Step Row ─────────────────────────────────────────────────────────────────
function StepRow({
  step, onChange, onDelete, onToggle, onOpenEmployees, statusOptions, urgencyOptions, onWarn,
}: {
  step: PlanningStep;
  ...
  const handleStartDateChange = (v: string) => {
    const end = toInputDate(step.endDate);
    if (end && v > end) { onWarn('תאריך התחלה לא יכול להיות גדול מתאריך סיום'); return; }
    onChange('startDate', v);
    if (end) {
      const diff = Math.max(1, dateDiffDays(v, end) + 1); // inclusive
      onChange('duration', diff);
    }
  };

  // 1+3 — end date validation
  const handleEndDateChange = (v: string) => {
    ...
    onChange('endDate', v);
    if (start) {
      const diff = Math.max(1, dateDiffDays(start, v) + 1); // inclusive
      onChange('duration', diff);
    }
  };

  // 5+7 — duration validation
  const handleDurationChange = (v: number) => {
    const newDuration = Math.max(1, Math.floor(v)); // 5: min=1
    const start = toInputDate(step.startDate);
  ...
  return (
    <div className="border-2 border-blue-200 rounded-xl overflow-hidden bg-white shadow-sm">
      <div className="flex items-center gap-1.5 px-2 py-2 bg-blue-50">
        ...
        <div className="w-10 text-center text-xs font-bold text-gray-500 bg-white border border-gray-200 rounded-lg py-1.5 shrink-0">{step.orderNum}</div>
        <input type="text" value={step.name} onChange={e => onChange('name', e.target.value)}
          className="min-w-0 flex-1 px-2 py-1.5 border border-gray-300 rounded-lg text-xs font-semibold focus:ring-2 focus:ring-blue-300" placeholder="שם שלב"/>
        ...
        <div className="w-16 shrink-0 flex items-center justify-center">
          <input
            ...
            title={(step.orderNum ?? 0) === 1 ? 'לא ניתן לערוך בשורה הראשונה' : 'תלוי שלב'}
          />
        </div>
        ...
        <div className="w-28 shrink-0 flex flex-col gap-1">
          ...
          />
          {(() => {
            const st = statusOptions.find(s => s.id === step.statusId);
            const pct = st?.progressPercentage ?? 0;
            const color = st?.color ?? '#10b981';
            return (
...
// ─── Task Row ─────────────────────────────────────────────────────────────────
function TaskRow({
  task, onChange, onDelete, onOpenEmployees, statusOptions, urgencyOptions, onWarn, stepDuration,
}: {
  task: PlanningTask;
  steps: PlanningStep[];
  onChange: (field: string, value: any) => void;
  ...
  const handleStartDateChange = (v: string) => {
    const end = toInputDate(task.endDate);
    if (end && v > end) { onWarn('תאריך התחלה לא יכול להיות גדול מתאריך סיום'); return; }
    onChange('startDate', v);
    if (end) {
      const diff = Math.max(1, dateDiffDays(v, end) + 1); // inclusive
  ...
  // 6+8 — duration validation
  const handleDurationChange = (v: number) => {
    const newDuration = Math.max(1, Math.floor(v)); // 6: min=1
    if (newDuration > stepDuration) {
      onWarn(`משך זמן המשימה (${newDuration}) לא יכול לעבור את משך זמן השלב (${stepDuration})`);
      return;
    }
    const start = toInputDate(task.startDate);
    const end   = toInputDate(task.endDate);
    if (start && end) {
      const diff = dateDiffDays(start, end) + 1;
      if (newDuration < diff) {
        // 8: adjust endDate
        const newEnd = addDays(start, newDuration - 1);
        onWarn(`משך הזמן קטן מהפרש התאריכים — תאריך הסיום יעודכן ל-${newEnd.split('-').reverse().join('/')}`);
        onChange('duration', newDuration);
        onChange('endDate', newEnd);
        return;
      }
  ...
  return (
    <div className="flex items-center gap-1.5 px-2 py-1.5 bg-white rounded-lg border border-purple-100">
      ...
      <input type="text" value={task.name} onChange={e => onChange('name', e.target.value)}
        className="min-w-0 flex-1 px-2 py-1.5 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-purple-300" placeholder="שם משימה"/>
      <div className="flex items-center gap-0.5 w-20 shrink-0">
      ...
      <div className="w-16 shrink-0 flex items-center justify-center">
        <input
          ...
          title={(task.orderNum ?? 0) === 1 ? 'לא ניתן לערוך בשורה הראשונה' : 'תלוי משימה'}
        />
      </div>
      <button onClick={onOpenEmployees}
      ...
      <div className="w-28 shrink-0 flex flex-col gap-1">
        ...
        />
        {(() => {
          const st = statusOptions.find(s => s.id === task.statusId);
          const pct = st?.progressPercentage ?? 0;
          const color = st?.color ?? '#10b981';
          return (
            <div className="flex items-center gap-1.5 px-0.5">
...
function ColHeaders({ forStep }: { forStep?: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-1.5 px-2 py-2 rounded-lg text-xs font-bold text-gray-700 text-right ${forStep ? 'bg-blue-100 pr-9' : 'bg-purple-100'}`}>
      {cols.map((c, i) => <div key={i} className={`${c.w} shrink-0 text-right`}>{c.label}</div>)}
    </div>
  );
}
...
export default function ProjectTopicsTab({ projectId }: ProjectTopicsTabProps) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      } catch { setSaveError('שגיאה בטעינת תכנון הפרויקט'); }
    };
    load();
  }, [projectId]);
  ...
  const addStep = (subjectId: number) => {
    ...
    setSubjects(p => p.map(s => s.id !== subjectId ? s : {
      ...s, steps: [...s.steps, {
        ...
        orderNum: s.steps.length + 1, percentage: 0, workHours: 0, workDays: 0, duration: 1,
    ...
    setSubjects(p => p.map(s => s.id !== subjectId ? s : {
      ...s, steps: s.steps.map(st => st.id !== stepId ? st : {
        ...
        tasks: [...st.tasks, {
          ...
          orderNum: st.tasks.length + 1, percentage: 0, workHours: 0, workDays: 0,
          duration: Math.max(1, st.duration || 1),
  ...
  const saveStepEmployees = (subjectId: number, stepId: number, emps: EmployeeLink[]) => {
    setSubjects(p => p.map(s => s.id !== subjectId ? s : { ...s, steps: s.steps.map(st => st.id !== stepId ? st : { ...st, employees: emps }) }));
    dirty();
  };
  ...
  return (
    <div className="space-y-4" dir="rtl">
        ...
        <div key={subject.id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-2.5 bg-amber-50 border-b border-amber-200">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer shrink-0 text-xs text-gray-600">
              <input type="checkbox" checked={subject.isActive} onChange={() => updateSubject(subject.id, 'isActive', !subject.isActive)} className="w-4 h-4 accent-emerald-500"/>פעיל
            </label>
      ...
      {openImport && (
        <ImportSubjectTemplatesModal projectId={projectId!} onImport={() => { setSubjects([]); }} onClose={() => setOpenImport(false)}/>
      )}
