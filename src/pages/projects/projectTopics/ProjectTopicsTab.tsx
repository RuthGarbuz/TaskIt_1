...
import type { EmployeeLink, PlanningStep, PlanningSubject, PlanningTask, SubContract, SubContractLink } from '../../../Data/projectsData';
...
//   onSave: (items: SubContractLink[]) => void;
//   onClose: () => void;
...
//         : [...prev, { id: item.id, name: item.name, subjectId }]
//     );
...
//           {subContractsOptions.map(sc => (
//             <label
//               key={sc.id}
//               className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${
//                 selectedIds.has(Number(sc.id))
...
//               <input
//                 type="checkbox"
//                 checked={selectedIds.has(Number(sc.id))}
//                 onChange={() => toggle(sc)}
...
//             className="flex-1 bg-emerald-500 text-white py-2.5 rounded-lg text-sm font-bold hover:bg-emerald-600"
//           >
...
function InlineSelect({
  value, options, onChange, colorMap, width = 'w-24',
}: {
  value: number;
  ...
  return (
    <select
      ...
      className={`${width} px-1.5 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-indigo-300 ${
    ...
    <div className="border-2 border-blue-200 rounded-xl overflow-hidden bg-white shadow-sm">
      ...
      <div className="flex items-center gap-1.5 px-2 py-2 bg-blue-50">
        ...
        <div className="w-36 shrink-0 relative">
          <input type="date" value={toInputDate(step.startDate)}
            onChange={e => onChange('startDate', e.target.value)}
            className="w-full px-1.5 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-blue-300"
          />
          {/* {!step.startDate && (
            <span className="absolute inset-0 flex items-center pr-2 text-xs text-gray-400 pointer-events-none">התחלה</span>
          )} */}
        </div>

        {/* תאריך סיום */}
        <div className="w-36 shrink-0 relative">
          <input type="date" value={toInputDate(step.endDate)}
            onChange={e => onChange('endDate', e.target.value)}
            className="w-full px-1.5 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-blue-300"
          />
          {/* {!step.endDate && (
            <span className="absolute inset-0 flex items-center pr-2 text-xs text-gray-400 pointer-events-none">סיום</span>
          )} */}
        </div>
        ...
        {/* סטטוס */}
        <InlineSelect
          value={step.statusId} options={statusOptions}
          onChange={v => onChange('statusId', v)}
          colorMap={STATUS_COLORS} width="w-24"
        />

        {/* עדיפות */}
        <InlineSelect
          value={step.urgencyId} options={urgencyOptions}
          onChange={v => onChange('urgencyId', v)}
          colorMap={URGENCY_COLORS} width="w-24"
        />
        ...
        <div className="w-10 shrink-0 flex items-center justify-center">
          <input
            ...
            onChange={() => onChange('isActive', !step.isActive)}
            className="w-4 h-4 accent-emerald-500"
            title="פעיל"
          />
        </div>
...
function TaskRow({
  ...
  return (
    <div className="flex items-center gap-1.5 px-2 py-1.5 bg-white rounded-lg border border-purple-100">
      ...
      {/* תאריך התחלה */}
      <div className="w-36 shrink-0 relative">
        <input type="date" value={toInputDate(task.startDate)}
          onChange={e => onChange('startDate', e.target.value)}
          className="w-full px-1.5 py-1.5 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-purple-300"
        />
        {/* {!task.startDate && (
          <span className="absolute inset-0 flex items-center pr-2 text-xs text-gray-400 pointer-events-none">התחלה.ת</span>
        )} */}
      </div>

      {/* תאריך סיום */}
      <div className="w-36 shrink-0 relative">
        <input type="date" value={toInputDate(task.endDate)}
          onChange={e => onChange('endDate', e.target.value)}
          className="w-full px-1.5 py-1.5 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-purple-300"
        />
        {/* {!task.endDate && (
      ...
      >
        <Users size={11}/>
        עובדים
        {task.employees.filter(employee => !employee.isDeleted).length > 0 && (
          <span className="bg-white text-purple-700 rounded-full w-4 h-4 flex items-center justify-center text-[10px] font-bold">
            {task.employees.filter(employee => !employee.isDeleted).length}
          </span>
        )}
      </button>

      {/* סטטוס */}
      <InlineSelect value={task.statusId} options={statusOptions}
        onChange={v => onChange('statusId', v)} colorMap={STATUS_COLORS} width="w-24"/>

      {/* עדיפות */}
      <InlineSelect value={task.urgencyId} options={urgencyOptions}
        onChange={v => onChange('urgencyId', v)} colorMap={URGENCY_COLORS} width="w-24"/>
      ...
      <div className="w-10 shrink-0 flex items-center justify-center">
        <input
          ...
          onChange={() => onChange('isActive', !task.isActive)}
          className="w-4 h-4 accent-emerald-500"
          title="פעיל"
        />
      </div>
...
const [messageBox, setMessageBox] = useState<{
  ...
  // ── Load planning hierarchy ───────────────────────────────────────────────
  useEffect(() => {
    const loadPlanningHierarchy = async () => {
      if (!projectId) {
        setSubjects([]);
        return;
      }
      ...
      try {
        ...
        // const nextLinks = data.subContracts ?? nextSubjects.reduce<Record<number, SubContractLink[]>>((acc, s) => {
        //   acc[s.id] = s.subContractsLink ?? [];
        //   return acc;
        // }, {});
        //
    ...
    loadPlanningHierarchy();
  }, [projectId]);

  useEffect(() => {
    const loadPlanningSystemLists = async () => {
      try {
        ...
        console.error('Failed to load planning system lists:', error);
      }
    };

    loadPlanningSystemLists();
  }, []);

  // ── Filtered subjects ──────────────────────────────────────────────────────
  const filtered = useMemo(() => {
    const visibleSubjects = subjects.filter(s => !s.isDeleted);
    if (!search.trim()) return visibleSubjects;
  ...
  const getDefaultId = (options: SelectOption[]) =>
  ...
  const getEmployeeKey = (employee: EmployeeLink) =>
    employee.linkId ? `link-${employee.linkId}` : `emp-${employee.id}`;

  const mergeEmployeeLinks = (current: EmployeeLink[], next: EmployeeLink[]): EmployeeLink[] => {
    const nextMap = new Map(next.map(emp => [getEmployeeKey(emp), emp]));
    const currentMap = new Map(current.map(emp => [getEmployeeKey(emp), emp]));
    const merged: EmployeeLink[] = [];

    for (const [key, nextEmp] of nextMap) {
    ...
    for (const [key, existing] of currentMap) {
      if (!nextMap.has(key)) {
        if (existing.isNew || isTempId(existing.linkId || existing.id)) {
          continue;
        }
  ...
  const mergeSubContractLinks = (current: SubContractLink[], next: SubContractLink[]): SubContractLink[] => {
    const nextIds = new Set(next.map(link => link.id));
    const currentMap = new Map(current.map(link => [link.id, link]));
  ...
  const updateStep = (subjectId: number, stepId: number, field: string, val: any) => {
     if (field === 'percentage') {
                ...
                const otherSum = (subjects.find(s => s.id === subjectId)?.steps ?? [])
                  .filter((x: PlanningStep) => x.id !== stepId)
                  .reduce((acc: number, x: PlanningStep) => acc + x.percentage, 0);
                ...
                //return { ...st, percentage: newPct };
              }
    setSubjects(p => p.map(s => s.id !== subjectId ? s : {
      ...s, steps: s.steps.map(st => st.id !== stepId ? st : { ...st, [field]: val, isModified: true }),
    }));
    dirty();
  };
...
// ...existing code...
  const updateTask = async (
    ...
    if (field === 'workHours') {
      ...
      const otherH = step.tasks.filter(x => x.id !== taskId && !x.isDeleted)
        .reduce((a, x) => a + x.workHours, 0);
      if (otherH + newH > step.workHours) {
        const yes = await openConfirm(
          `סה"כ השעות במשימות (${(otherH + newH).toFixed(2)}) גדול משעות השלב (${step.workHours}).\n\nהאם לעדכן את שעות השלב?`
        );
        if (!yes) return;
    ...
    setSubjects(p => p.map(s =>
      s.id !== subjectId ? s : {
        ...
        steps: s.steps.map(st =>
          st.id !== stepId ? st : {
            ...st,
            ...(updatedStepHours !== null
              ? { workHours: updatedStepHours, workDays: updatedStepHours / WORK_HOURS_PER_DAY }
              : {}),
            tasks: st.tasks.map(t => {
              ...
              if (field === 'duration') {
                ...
                if (newDuration > st.duration) {
                  setMessageBox({
                    isOpen: true,
                    title: 'אזהרה',
                    message: `משך זמן המשימה (${newDuration}) לא יכול לעבור את משך זמן השלב (${st.duration}).`,
                    type: 'warning'
                  });
                  return t;
                }
              ...
              if (field === 'percentage') {
                const newPct = Math.max(0, Math.min(100, Number(val)));
                const otherSum = st.tasks.filter(x => x.id !== taskId && !x.isDeleted)
                ...
                return { ...t, percentage: newPct, isModified: !t.isNew };
              }
              // if (field === 'dependsOnTaskId') {
              //   return { ...t, dependsOnTaskId: Number(val), isModified: !t.isNew };
              // } 
...
// ...existing code...
  // const updateTask = (subjectId: number, stepId: number, taskId: number, field: string, val: any) => {
  //   setSubjects(p => p.map(s => s.id !== subjectId ? s : {
  //     ...s, steps: s.steps.map(st => st.id !== stepId ? st : {
  //       ...st, tasks: st.tasks.map(t => t.id !== taskId ? t : { ...t, [field]: val, isModified: true }),
  //     }),
  //   }));
  //   dirty();
  // };
  ...
  const deleteTask = (subjectId: number, stepId: number, taskId: number) => {
    setSubjects(p => p.map(s => s.id !== subjectId ? s : {
      ...s, steps: s.steps.map(st => st.id !== stepId ? st : {
        ...st,
        tasks: st.tasks.flatMap(t => {
          if (t.id !== taskId) return [t];
          if (isTempId(t.id) || t.isNew) {
            return [];
          }
  ...
  const saveStepEmployees = (subjectId: number, stepId: number, emps: EmployeeLink[]) => {
    setSubjects(p => p.map(s => s.id !== subjectId ? s : {
      ...s, steps: s.steps.map(st => st.id !== stepId ? st : { ...st, employees: mergeEmployeeLinks(st.employees, emps) }),
    }));
    dirty();
  };
  ...
  const saveTaskEmployees = (subjectId: number, stepId: number, taskId: number, emps: EmployeeLink[]) => {
    setSubjects(p => p.map(s => s.id !== subjectId ? s : {
      ...s, steps: s.steps.map(st => st.id !== stepId ? st : {
        ...st, tasks: st.tasks.map(t => t.id !== taskId ? t : { ...t, employees: mergeEmployeeLinks(t.employees, emps) }),
      }),
    }));
    dirty();
  };

  // ── Employee modal props ───────────────────────────────────────────────────
...
setOpenImport(true);
  ...
  return (
    <div className="space-y-4" dir="rtl">
      ...
      >
        {saving ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>שומר...</>
         : <><Save size={15}/>שמור הכל</>}
      </button>
      </div>
      ...
      <div key={subject.id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <button onClick={() => updateSubject(subject.id, 'isExpanded', !subject.isExpanded)}
          className="p-1 hover:bg-amber-200 rounded-lg shrink-0">
        ...
        <span className={`text-xs font-bold px-2.5 py-1 rounded-full shrink-0 ${
          subject.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'
        }`}>
          {subject.isActive ? 'פעיל' : 'לא פעיל'}
        </span>

        <label className="flex items-center gap-1.5 cursor-pointer shrink-0 text-xs text-gray-600">
          <input type="checkbox" checked={subject.isActive}
        ...
        <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-semibold shrink-0">
          {subject.steps.filter(step => !step.isDeleted).length} שלבים
        </span>

        {/* קישור לתתי חוזים */}
        <button
          onClick={() => setSubModal({ subjectId: subject.id, name: subject.name })}
        ...
        >
          <Link size={13}/>
          תתי חוזים
          {( subject.subContractsLink?.filter(link => !link.isDeleted).length  ?? 0) > 0 && (
          <span className="bg-indigo-600 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] font-bold">
            {subject.subContractsLink?.filter(link => !link.isDeleted).length ?? 0}
          </span>
          )}
        </button>
        ...
        <div className="p-3 space-y-2 bg-gray-50">
          ...
          <div className="min-w-max space-y-2">
            ...
            {subject.steps.some(step => !step.isDeleted) && (
            <ColHeaders forStep />
            )}

            {subject.steps.filter(step => !step.isDeleted).map(step => (
            <div key={step.id}>
              <StepRow
              step={step}
              ...
              <div className="mr-6 mt-1 space-y-1 p-2 bg-purple-50 rounded-xl border border-purple-200">
                {step.tasks.some(task => !task.isDeleted) && (
                <>
                  <ColHeaders />
                  {step.tasks.filter(task => !task.isDeleted).map(task => (
                  <TaskRow
                    key={task.id}
                    task={task}
                  ...
                  <div className="overflow-x-auto">
                    ...
                    <div className="grid grid-cols-[40px_1fr_80px_64px_64px_56px_144px_144px_112px_80px_96px_96px_40px_18px] gap-2 items-center px-2 py-2">
                      <div />
                      <div className="text-right font-bold text-purple-800">סה"כ משימות</div>
                      <div className="text-center font-bold text-purple-800 bg-purple-100 rounded py-1">
                      {step.tasks.filter(task => !task.isDeleted).reduce((s, t) => s + t.percentage, 0).toFixed(1)}%
                      </div>
                      <div className="text-center font-bold text-purple-800 bg-purple-100 rounded py-1">
                      {step.tasks.filter(task => !task.isDeleted).reduce((s, t) => s + t.workHours, 0).toFixed(1)}
                      </div>
                      <div className="text-center font-bold text-purple-800 bg-purple-100 rounded py-1">
                      {step.tasks.filter(task => !task.isDeleted).reduce((s, t) => s + t.workDays, 0).toFixed(1)}
                      </div>
                      <div className="text-center font-bold text-purple-800 bg-purple-100 rounded py-1">
                      {step.tasks.filter(task => !task.isDeleted).reduce((s, t) => s + t.duration, 0).toFixed(1)}
                      </div>
                ...
                <button
                onClick={() => addTask(subject.id, step.id)}
            ...
            <div className="border-2 border-emerald-300 rounded-lg bg-emerald-50 p-2">
              ...
              <div className="min-w-max">
                ...
                <div />
                <div />
                <div className="text-right font-bold text-emerald-800">סה"כ</div>
                <div className="text-center font-bold text-emerald-800 bg-emerald-100 rounded py-1">
                  {subject.steps.filter(step => !step.isDeleted).reduce((s, st) => s + st.percentage, 0).toFixed(1)}%
                </div>
                <div className="text-center font-bold text-emerald-800 bg-emerald-100 rounded py-1">
                  {subject.steps.filter(step => !step.isDeleted).reduce((s, st) => s + st.workHours, 0).toFixed(1)}
                </div>
                <div className="text-center font-bold text-emerald-800 bg-emerald-100 rounded py-1">
                  {subject.steps.filter(step => !step.isDeleted).reduce((s, st) => s + st.workDays, 0).toFixed(1)}
                </div>
                <div className="text-center text-gray-400">-</div>
                <div className="text-center text-gray-400">-</div>
                <div className="text-center text-gray-400">-</div>
      ...
      {subModal && (
        ...
        linkedIds={subjects.find(s => s.id === subModal.subjectId)?.subContractsLink?.filter(link => !link.isDeleted) ?? []}
        onSave={items => {
      ...
      {openImport && (
        ...
        //     subContractsLink: template.subContractsLink ?? [],
        //     name: template.name ?? `נושא תכנון ${prev.length + index + 1}`
        //   }))
        // ]);
        // dirty();
        // setOpenImport(false);
