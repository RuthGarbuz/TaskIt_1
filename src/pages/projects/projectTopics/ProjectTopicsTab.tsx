...
import {
  ...
  Search, Link, GripVertical,
} from 'lucide-react';
import type { EmployeeLink, PlanningStep, PlanningSubject, PlanningTask, SubContract, SubContractLink, SubjectTemplate, TemplateStep, TemplateTask, SystemTable } from '../../../Data/projectsData';
...
/** Positive integers proportional to weights, each >= 1, sum === targetSum. */
const distributeProportionalInts = (weights: number[], targetSum: number): number[] | null => {
  const n = weights.length;
  if (n === 0) return [];
  if (targetSum < n) return null;
...
export default function ProjectTopicsTab({ projectId }: ProjectTopicsTabProps) {
  ...
  const handleImport = ({ templates: toImport, startDateByTemplateId }: ImportPlanningSubjectsPayload) => {
...
function SubjectBlock({
  subject, statusOptions, urgencyOptions,
