...
import {
  ...
  Users, TrendingUp, X, Lightbulb, RefreshCw, AlertCircle, Loader2, BarChart3,
} from 'lucide-react';
import type { MonthlyEmployee, WeeklyEmployee } from './Workload.types';
import { useMonthlyWorkload, useWeeklyWorkload } from './Useworkload';


// ─── Config ───────────────────────────────────────────────────────────────────

// TODO: replace with your actual auth source (Context / localStorage / etc.)
const EMPLOYEE_ID     = 1;
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
      ...
      <button onClick={onRetry}
        className="flex items-center gap-1 px-3 py-1.5 bg-red-100 hover:bg-red-200 rounded-lg font-semibold transition-all">
...
function WeeklyView({ employees, weekDays, colorFn }: {
            ...
            return (
              <tr key={emp.employee.id} className="hover:bg-gray-50">
                ...
                {weekDayStrs.map((dateStr, i) => {
                  const cell = emp.days.find(d => d.date === dateStr) ?? { hours: 0, percentage: 0, netCapacity: 0, date: dateStr };
                  return <WorkloadCell key={i} pct={cell.percentage} hours={cell.hours} colorFn={colorFn} />;
                })}
                <td className={`border border-gray-200 px-3 py-2 text-center ${colorFn(Math.round(emp.weeklyWorkloadPct))}`}>
...
function MonthlyView({ employees, colorFn }: {
        ...
        <tbody>
          {employees.map(emp => (
            <tr key={emp.employee.id} className="hover:bg-gray-50">
              ...
              {emp.months.map((m, i) => (
                <WorkloadCell key={i} pct={m.percentage} hours={m.hours} colorFn={colorFn} />
              ))}
              <td className={`border border-gray-200 px-3 py-2 text-center ${colorFn(emp.avgWorkloadPct)}`}>
                <div className="text-sm font-bold">{emp.avgWorkloadPct > 0 ? `${emp.avgWorkloadPct}%` : ''}</div>
                <div className="text-xs opacity-80">{emp.yearlyTaskHours > 0 ? `ש׳ ${emp.yearlyTaskHours}` : ''}</div>
              </td>
            </tr>
          ))}
        </tbody>
...
export default function WorkloadView() {
  ...
  // ── data ──
  const weekly  = useWeeklyWorkload(EMPLOYEE_ID, PERMISSION_TYPE, weekRef);
  const monthly = useMonthlyWorkload(EMPLOYEE_ID, PERMISSION_TYPE, year);

  const activeData   = viewMode === 'weekly' ? weekly  : monthly;
  const rawEmployees = viewMode === 'weekly'
    ? (weekly.data  as WeeklyEmployee[])
    : (monthly.data as MonthlyEmployee[]);
  ...
  const filteredEmployees = useMemo(() =>
    rawEmployees.filter(emp => {
      ...
      const matchesFilter = selectedIds.length === 0 || selectedIds.includes(emp.employee.id);
      return matchesSearch && matchesFilter;
    }),
    [rawEmployees, searchQuery, selectedIds]
  );

  // all unique employees for filter modal (use whichever list is loaded)
  const allEmployees = rawEmployees.map(e => e.employee);
  ...
  return (
    <div className="p-6 space-y-5" dir="rtl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="רענן נתונים">
            <RefreshCw size={16} className={activeData.loading ? 'animate-spin' : ''} />
          </button>
      ...
      {/* Error banner */}
      {activeData.error && <ErrorBanner message={activeData.error} onRetry={activeData.refetch} />}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          {viewMode === 'monthly' && (
            <div className="flex items-center gap-2 mr-auto">
              <button onClick={() => setYear(y => y - 1)} className="p-2 hover:bg-gray-100 rounded-lg">
                <ChevronRight size={18} className="text-gray-600" />
              </button>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        {activeData.loading && <LoadingOverlay />}

        {!activeData.loading && viewMode === 'weekly' && (
          <WeeklyView
            employees={filteredEmployees as WeeklyEmployee[]}
            weekDays={weekDays}
            colorFn={scheme.fn}
          />
        )}
        {!activeData.loading && viewMode === 'monthly' && (
          <MonthlyView
            employees={filteredEmployees as MonthlyEmployee[]}
            colorFn={scheme.fn}
          />
        )}
        {!activeData.loading && filteredEmployees.length === 0 && !activeData.error && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col" style={{ maxHeight: '80vh' }}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              {selectedIds.length > 0 && (
                <p className="text-xs text-indigo-600 font-semibold">{selectedIds.length} עובדים נבחרו</p>
              )}
            </div>
            <div className="flex-shrink-0 border-t px-6 py-4 flex gap-3">
              <button onClick={() => { setSelectedIds([]); setSearchQuery(''); }}
